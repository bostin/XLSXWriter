...
class XLSXWriter
{
    ...
    public function outputToBrowser($filename)
    {
        $headers = [
            ...
            'Content-Disposition' => 'attachment; filename="' . self::sanitize_filename($filename) . '.xlsx"',
    ...
    protected function initializeSheet($sheet_name)
    {
        ...
        $this->sheets[$sheet_name] = (object) array(
            ...
            'row_count'          => 0,
            'file_writer'        => new XLSXWriter\BuffererWriter($sheet_filename),
            'columns'            => [],
            'merge_cells'        => [],
    ...
    protected function writeCell(XLSXWriter\BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
    {
    ...
    protected function writeStylesXML()
    {
        ...
        $file               = new XLSXWriter\BuffererWriter($temporary_filename);
    ...
    private static function numberFormatStandardized($num_format)
    {
        ...
        elseif ($num_format == 'integer') $num_format = '0';
        elseif ($num_format == 'date') $num_format = 'YYYY-MM-DD';
        elseif ($num_format == 'datetime') $num_format = 'YYYY-MM-DD HH:MM:SS';
        elseif ($num_format == 'price') $num_format = '#,##0.00';
        elseif ($num_format == 'dollar') $num_format = '[$$-1009]#,##0.00;[RED]-[$$-1009]#,##0.00';
        elseif ($num_format == 'euro') $num_format = '#,##0.00 [$€-407];[RED]-#,##0.00 [$€-407]';
        $ignore_until = '';
        $escaped      = '';
